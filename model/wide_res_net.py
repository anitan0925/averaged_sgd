...
class WideResNet(nn.Module):
    def __init__(self, depth: int, width_factor: int, dropout: float, in_channels: int, labels: int):
        ...
        self._initialize(labels)

    def _initialize(self,labels):
        for m in self.modules():
            if isinstance(m, nn.Conv2d):
                ...
                nn.init.uniform_(m.weight.data,a = -1/np.sqrt(labels),b = 1/np.sqrt(labels))
                #m.weight.data.zero_()
                m.bias.data.zero_()
...
def WRN28_2(labels):
    return WideResNet(28,2,0,in_channels=3,labels=labels)

def WRN28_10(labels):
    return WideResNet(28,10,0,in_channels=3,labels=labels)